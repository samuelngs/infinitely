...
export default class ImageZoom extends Component {
  ...
  renderZoomed() {
    ...
    const { zoomImage, video } = this.props;
...
class Zoom extends Component {
  ...
  componentDidMount() {
    ...
    this.addListeners()
    if (this.state.video) setTimeout(() => this.setState({ ready: true }), 500);
  }
  ...
  componentDidUpdate(props, { ready: prev }) {
    const { ready: next, video } = this.state;
    if ( this.video && video && prev !== next ) {
      this.video.play();
    }
  }

  render() {
    const { src, video, isZoomed, ready } = this.state;
    const { alt, className, defaultStyles } = this.props;
    return (
