...
class EmojiRain extends Component {
  ...
  __paintEmoji(opts) {
    ...
    if (emoji.y >= this.canvas.height || emoji.opacity < 0.1) {
      ...
      emoji.y += emoji.speed;
      emoji.opacity -= emoji.opacitySpeed * 0.1;
    }
    ...
    if (this.useTwemoji && emoji.img && emoji.img !== '') {
      const size = isEven ? 30 : 40;
      context.drawImage(emoji.img, emoji.x, emoji.y, size, size);
    } else {
      context.font = isEven ? '30px serif' : '40px serif';
  ...
  giveMeARandomEmoji() {
    ...
    emoji.speed = 4 + Math.floor(Math.random() * 6 + 1);
    emoji.opacity = 1;
  ...
  render() {
    return (
      <div style={styles.container}>
        <canvas ref={el => this.canvas = el} style={styles.canvas} className="js-canvas" />
      </div>
    );
  }
}
